...
Navigate to **Settings** to establish the "Face" of your Fortress. These fields are used to populate the headers and footers of all issued PDFs (Quotes, Invoices, and Expenses).
...
*   **Doc Padding:** Determines the length of the sequential part of your document IDs (e.g., Padding of 3 results in `CDX-2620011`).
*   **Net Days (Invoices):** Set default "Net Days" that will pre-populate new documents.
*   **Quote Expiry (Days):** Set default validity period for new quotes, used to automatically calculate the expiration date.
*   **Default document Terms:** Standard terms that pre-populates the "Terms & Notes" section in PDFs (Quotes, Invoices, and Expenses).
...
The Product Catalog is the source of truth for your Sales documents.

*   **Catalog Number:** Your internal SKU or part number.
*   **Default Unit Price:** Pre-populates Quotes and Purchase Orders (can be overridden).
*   **Document Placement:** **(Critical)**
    *   `Standard Line Item`: Standard goods/services.
