...
### CODEX (The Narrative)
An unbroken record of the document lifecycle. Every transaction orbits a unique **Order Registry ID (CDX)**. By centralizing the deal history, Codexmetry allows you to visualize the entire path of a sale from the initial proposal to the final bank confirmation within a single order history and timeline.

### METRY (The Math)
A precision representation of account health through comprehensive analytics and financial statements. The system provides business intelligence by isolating economic performance (**Accrual**) from cash liquidity (**Cash**), supported by 60-month linear trends and interactive performance rankings for clients and products.
...
- **[System Maintenance](./guides/maintenance.md)**: Database optimization and snapshot management.
